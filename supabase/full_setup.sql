...
insert into procedures (kode, no, nama, target_min, satuan, peran_disyaratkan, syarat_tambahan, perlu_verifikasi) values ('PR-02', 2, 'Ultrasonografi ginekologi onkologi (TA/TV, IOTA/ADNEX, Doppler)', 3, 'pemeriksaan + laporan', 'mandiri', 'Angka ''≥3'' kemungkinan salah ekstraksi (mungkin ≥30). Cek PDF.', true) on conflict (kode) do update set nama=excluded.nama, target_min=excluded.target_min, satuan=excluded.satuan, peran_disyaratkan=excluded.peran_disyaratkan, syarat_tambahan=excluded.syarat_tambahan, perlu_verifikasi=excluded.perlu_verifikasi;
